...
module suncore {
    ...
    export interface ITimerManager {
         ...
         * @args[]: 参数列表，默认为: null
         * @loops: 循环设定次数，默认为: 1
         * @jumpFrame: 是否计算真实次数，默认为: false
         * @timerId: 定时器编号，请勿擅自传入此参数，防止定时器工作出错，默认为: 0
         * @createTime: 定时器的创建时间，请勿擅自传入此参数，防止定时器工作出错，默认为: -1
         * @timeout: 定时器上次响应时间，请勿擅自传入此参数，防止定时器工作出错，默认为: -1
         * @count: 当前重复次数，默认为: 0
         */
        addTimer(mod: ModuleEnum, delay: number | number[], method: Function, caller: Object, args?: any[], loops?: number, jumpFrame?: boolean, timerId?: number, createTime?: number, timeout?: number, count?: number): number;
